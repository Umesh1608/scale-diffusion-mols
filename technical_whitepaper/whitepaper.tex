...
\begin{equation}\label{eq:laplacian}
    \Lap = \degree - \adj,
\end{equation}
where $\degree = \mathrm{diag}(d_1, \ldots, d_N)$ is the degree matrix with $d_i = \sum_j A_{ij}$. The normalized Laplacian is:
\begin{equation}\label{eq:norm_laplacian}
    \Lap_{\mathrm{norm}} = \degree^{-1/2} \Lap\, \degree^{-1/2} = \bm{I} - \degree^{-1/2} \adj\, \degree^{-1/2}.
\end{equation}
...
\State Construct adjacency $\adj$ and degree $\degree$ matrices from $\cG$
\State Compute Laplacian $\Lap \gets \degree - \adj$
...
\For{$k = 1, 2, \ldots, L$}
    ...
    \State Construct coarsened adjacency: $\adj^{(k)}_{IJ} \gets \mathbbm{1}\!\left[\exists\, (i,j) \in E : c^{(k)}(i) = I, c^{(k)}(j) = J, I \neq J\right]$
\EndFor
...
\midrule
Atom Stability (\%) & $\frac{1}{N_{\text{gen}}} \sum_i \frac{1}{|m_i|} \sum_{a \in m_i} \mathbbm{1}[\text{valency}(a) = \text{target\_valency}(a)]$ \\
\midrule
Molecule Stability (\%) & $\frac{1}{N_{\text{gen}}} \sum_i \mathbbm{1}[\text{all atoms in } m_i \text{ are stable}]$ \\
\midrule
